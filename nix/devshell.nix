...
let
  ...
  rustcProfilingFlags = ''build.rustflags = ["-C", "force-frame-pointers=yes", "-C", "symbol-mangling-version=v0"]'';
  ...
  preparegrind = pkgs.writeShellScriptBin "preparegrind" ''
    ...
    cargo build --bin usbvfiod $CARGO_FLAGS --config ${lib.escapeShellArg rustcProfilingFlags}
  ...
  cachegrind = pkgs.writeShellScriptBin "cachegrind" ''
    ...
    ${pkgs.valgrind}/bin/valgrind --tool=cachegrind \
      ...
      --trace-children=yes \
      ./target/$BUILD/usbvfiod --socket-path ${vfio-user-socket} --hotplug-socket-path ${hotplug-socket} --no-color
  '';
