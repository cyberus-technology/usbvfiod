...
in
{
  windows-test = pkgs.testers.runNixOSTest {
    ...
    testScript = ''
      ...
      # when running the driver for testing, keep the driver from exiting to manually interact
      # this test will not be helpful if it succeeds with this testScript anyway
      machine.wait_until_succeeds("sleep 3600", timeout=3600)
    '';
