...
mod tests {
    ...
    fn parse_configure_endpoint_command_trb() {
        ...
        if let CommandTrbVariant::ConfigureEndpoint(data) = trb {
            assert_eq!(
                0x1122334455667780, data.input_context_pointer,
                "input_context_pointer was parsed incorrectly."
            );
            assert!(data.deconfigure, "deconfigure bit was parsed incorrectly.");
            assert_eq!(0x13, data.slot_id, "slot_id was parsed incorrectly.");
        } else {
            panic!(
                "A TRB with TRB type 12 should result in a CommandTrb::ConfigureEndpointCommand. Got instead: {:?}",
                trb
            );
        }
    }
