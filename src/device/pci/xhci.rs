    ...
    pub fn set_device(&mut self, device: Box<dyn RealDevice>) {
        if let Some(speed) = device.speed() {
            ...
            if self.running {
                ...
                let trb =
                    EventTrb::new_port_status_change_event_trb(available_port_index as u8 + 1);
    ...
    fn read_io(&self, region: u32, req: Request) -> u64 {
        ...
        match req.addr {
            ...
            val if val == offset::SUPPORTED_PROTOCOLS_USB2_CONFIG + 4 => {
                capability::supported_protocols_usb2::CONFIG >> 32
            }
