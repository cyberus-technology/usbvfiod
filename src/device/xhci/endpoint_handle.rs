...
pub mod tests {
    ...
    async fn control_request_returns_short_packet() {
        const SHORT_AFTER_BYTES: u16 = 256;
        ...
        assert_eq!(
            ...
            Some(EventTrb::new_transfer_event_trb(
                SECOND_ADDRESS,
                TRANSFER_LENGTH - SHORT_AFTER_BYTES as u32,
                CompletionCode::Success,
                false,
                ENDPOINT_ID,
                SLOT_ID,
    ...
    async fn long_control_request_returns_short_packet() {
        const SHORT_AFTER_BYTES: u16 = 512 * 2 + 128;
        let (mut interrupter, mut control_endpoint) = init_control_endpoint_handle_test(
            MockRealControlEndpointReadStatic::new(Some(SHORT_AFTER_BYTES)),
        );
        ...
        assert_eq!(
            ...
            Some(EventTrb::new_transfer_event_trb(
                FOURTH_ADDRESS,
                TRANSFER_LENGTH * 3 - SHORT_AFTER_BYTES as u32,
                CompletionCode::Success,
                false,
                ENDPOINT_ID,
                SLOT_ID,
    ...
    async fn long_control_request_returns_short_packet_and_has_subsequent_trb() {
        const SHORT_AFTER_BYTES: u16 = 512 + 128;
        let (mut interrupter, mut control_endpoint) = init_control_endpoint_handle_test(
            MockRealControlEndpointReadStatic::new(Some(SHORT_AFTER_BYTES)),
        );
        ...
        assert_eq!(
            ...
            Some(EventTrb::new_transfer_event_trb(
                THIRD_ADDRESS,
                TRANSFER_LENGTH * 2 - SHORT_AFTER_BYTES as u32,
                CompletionCode::Success,
                false,
                ENDPOINT_ID,
                SLOT_ID,
