...
impl<RIEH: RealInEndpointHandle> InEndpointHandle<RIEH> {
    ...
    fn handle_normal_trb_pre_hardware(
        ...
        // if buffer enough for smaller request block count then do smaller
        if (self.buffer.len() > 0)
            && (normal_trb_data.transfer_length as usize % 512 == self.buffer.len())
        {
            self.real_ep
                .submit(512 * (normal_trb_data.transfer_length as usize / 512))?;
        } else {
            self.real_ep
