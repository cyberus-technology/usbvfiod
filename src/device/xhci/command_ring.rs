...
use crate::device::{
    ...
    pci::{
        ...
        trb::{
            AddressDeviceCommandTrbData, CommandTrb, CommandTrbVariant, CompletionCode,
            ConfigureEndpointCommandTrbData, EvaluateContextCommandTrbData, EventTrb,
            SetTrDequeuePointerCommandTrbData, StopEndpointCommandTrbData,
        },
    },
    xhci::{linked_ring::LinkedRing, slot_manager::SlotManager},
};

use super::endpoint::EndpointMessage;
...
impl CommandWorker {
    ...
    async fn process_command(&self) {
        ...
        if let WorkerState::ProcessingCommand(trb) = &self.state {
            let completion_event = match &trb.variant {
                ...
                CommandTrbVariant::DisableSlot(data) => EventTrb::new_command_completion_event_trb(
                    trb.address,
                    0,
                ...
                CommandTrbVariant::StopEndpoint(data) => {
                    EventTrb::new_command_completion_event_trb(
                        trb.address,
                        0,
                        self.handle_stop_endpoint(data).await,
                        data.slot_id,
                    )
                }
                CommandTrbVariant::SetTrDequeuePointer(data) => {
                    EventTrb::new_command_completion_event_trb(
                        trb.address,
                        0,
                        self.handle_settrdequeuepointer(data).await,
                        data.slot_id,
                    )
                }
                CommandTrbVariant::ResetDevice(reset_device_command_trb_data) => todo!(),
                CommandTrbVariant::ForceHeader => todo!(),
                CommandTrbVariant::NoOp => todo!(),
                CommandTrbVariant::Unrecognized(_, trb_parse_error) => todo!(),
            };
            self.event_ring_sender.send(completion_event);
        }
    }

    fn handle_enable_slot(&self) -> (CompletionCode, u8) {
        self.slot_manager
            .write()
            // Safety: panicking when encountering poisened lock
            .unwrap()
            .allocate_slot()
            .map_or_else(
                || {
    ...
    fn handle_disable_slot(&self, slot_id: u8) -> CompletionCode {
        match self.slot_manager.write().unwrap().free_slot(slot_id) {
            Ok(_) => CompletionCode::Success,
            Err(_) => CompletionCode::SlotNotEnabledError,
        }
    }

    fn handle_address_device(&self, data: &AddressDeviceCommandTrbData) -> CompletionCode {
        match self.slot_manager.read().unwrap().slot_mut(data.slot_id) {
            Some(mut slot) => slot
                .handle_address_device(data.input_context_pointer, data.block_set_address_request),
            None => CompletionCode::SlotNotEnabledError,
        }
    }

    fn handle_configure_endpoint(&self, data: &ConfigureEndpointCommandTrbData) -> CompletionCode {
        match self.slot_manager.read().unwrap().slot_mut(data.slot_id) {
            Some(mut slot) => {
                slot.handle_configure_endpoint(data.input_context_pointer, data.deconfigure)
            }
            None => CompletionCode::SlotNotEnabledError,
        }
    }

    fn handle_evaluate_context(&self, data: &EvaluateContextCommandTrbData) -> CompletionCode {
        match self.slot_manager.read().unwrap().slot_mut(data.slot_id) {
            Some(slot) => slot.handle_evaluate_context(data.input_context_pointer),
            None => CompletionCode::SlotNotEnabledError,
        }
    }

    async fn handle_stop_endpoint(&self, data: &StopEndpointCommandTrbData) -> CompletionCode {
        let (sender, receiver) = oneshot::channel();

        {
            let slot_manager = self.slot_manager.read().unwrap();

            let slot = match slot_manager.slot_ref(data.slot_id) {
                Some(slot) => slot,
                None => return CompletionCode::SlotNotEnabledError,
            };

            slot.send_to_endpoint(data.endpoint_id, EndpointMessage::Stop(sender));
        }
    ...
    async fn handle_settrdequeuepointer(
        &self,
        data: &SetTrDequeuePointerCommandTrbData,
    ) -> CompletionCode {
        let (sender, receiver) = oneshot::channel();

        {
            ...
            slot.send_to_endpoint(
                data.endpoint_id,
                EndpointMessage::SetTrDequeuePointer(
                    data.dequeue_pointer,
