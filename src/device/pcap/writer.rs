...
///
/// This produces the bytes for a single PCAP record, which is one complete packet entry in the file.
...
/// Global entry point for optional USB PCAP logging.
///
/// Initialized once at startup when a path is provided. Callers can then
/// write PCAP records without owning a writer; if not initialized, writes
/// are ignored.
